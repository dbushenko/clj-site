...
;; <strong>try-parse-date</strong> tries to parse date or date time of the article.
;;
;; <i>Parameters:</i>
;; <ul>
;; <li>str-date -- the date in string format as described in *date-format* or *date-time-format*;</li>
;; </ul>
;; <i>Output:</i> java.util.Date object.
...
;; <strong>generate-rss</strong> generates rss for last :rss-count articles, stores it in rss.xml.
;; Will generate records only for articles containing date.
;;
;; <i>Parameters:</i>
;; <ul>
;; <li>cfg -- the configuration file;</li>
;; <li>posts-list -- the list of posts</li>
;; </ul>
...
(defn make-config [cfg dir-name]
  ...
  (-> cfg
      ...
      (assoc :rss-description (or (:rss-description cfg) "Generated by clj-site (https://github.com/dbushenko/clj-site)"))
      (assoc :rss-title (or (:rss-title cfg) "Static site"))
      (assoc :rss-link (or (:rss-link cfg) "https://github.com/dbushenko/clj-site"))
      (assoc :rss-count (or (:rss-count cfg) 10))))
...
(defn -main [ & [input-dir-name]]
  (if (nil? input-dir-name)
    ...
    (let [dir-name (add-separator input-dir-name)      ;; input directory name ending with file separator
      ...
      (generate-rss cfg posts-list))))     ;; generate RSS
